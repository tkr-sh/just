            ...
            match state {
                ...
                State::Backslash => {
                    match c {
                        ...
                        character => {
                            return Err(
                                token.error(CompileErrorKind::InvalidEscapeSequence { character })
                            )
                        },
                    }
                    state = State::Initial;
                ...
                State::Unicode => {
                    match c {
                        ...
                        character => {
                            return Err(
                                token.error(CompileErrorKind::UnicodeEscapeDelimiter { character })
                            );
                        },
                    }
                },
                    ...
                    match c {
                        '}' => {
                            ...
                            cooked.push(char::from_u32(codepoint).ok_or_else(|| {
                                token.error(CompileErrorKind::UnicodeEscapeRange {
                                    hex: hex.clone(),
                                })
                            })?);
                        ...
                        '0'..='9' | 'A'..='F' | 'a'..='f' => {
                            hex.push(c);
                            if hex.len() > 6 {
                                return Err(token.error(CompileErrorKind::UnicodeEscapeLength {
                                    hex: hex.clone(),
                                }));
                            }
                        },
                        _ => {
                            return Err(token
                                .error(CompileErrorKind::UnicodeEscapeCharacter { character: c }));
                        },
                    }
                },
            }
    ...
    fn parse_set(&mut self) -> CompileResult<'src, Set<'src>> {
        ...
        let set_bool = match keyword {
            ...
            Keyword::PositionalArguments => {
                Some(Setting::PositionalArguments(self.parse_set_bool()?))
            },
        ...
        let set_value = match keyword {
            ...
            Keyword::WorkingDirectory => {
                Some(Setting::WorkingDirectory(self.parse_string_literal()?))
            },
            _ => None,
        };
...
#[cfg(test)]
mod tests {
    use {super::*, pretty_assertions::assert_eq, CompileErrorKind::*};
