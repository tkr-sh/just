...
impl<'src, 'run> Evaluator<'src, 'run> {
        ...
        match expression {
            ...
            Expression::Call { thunk } => {
                ...
                let result = match thunk {
                    Nullary { function, .. } => {
                        function(function::Context::new(self, thunk.name()))
                    },
                    Unary { function, arg, .. } => {
            ...
            Expression::Join { lhs: None, rhs } => {
                Ok("/".to_string() + &self.evaluate_expression(rhs)?)
            },
            Expression::Join {
                lhs: Some(lhs),
                rhs,
    ...
    fn run_backtick(&self, raw: &str, token: &Token<'src>) -> RunResult<'src, String> {
        self.run_command(raw, &[]).map_err(|output_error| {
            Error::Backtick {
                token: *token,
                output_error,
            }
    ...
    pub fn run_command(&self, command: &str, args: &[&str]) -> Result<String, OutputError> {
        ...
        cmd.stderr(
            if self.context.config.verbosity.quiet() {
                Stdio::null()
            } else {
                Stdio::inherit()
            },
    ...
    pub fn evaluate_line(&mut self, line: &Line<'src>, continued: bool) -> RunResult<'src, String> {
