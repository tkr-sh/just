...
impl Subcommand {
            ...
            let fallback = justfile.settings.fallback &&
                matches!(
                    ...
                    SearchConfig::FromInvocationDirectory |
                        SearchConfig::FromSearchDirectory { .. }
                );

            let result = justfile.run(config, &search, overrides, arguments);

            if fallback {
                if let Err(err @ (Error::UnknownRecipe { .. } | Error::UnknownSubmodule { .. })) =
                    result
                {
    ...
    fn format(config: &Config, search: &Search, compilation: Compilation) -> RunResult<'static> {
        ...
        fs::write(&search.justfile, formatted).map_err(|io_error| {
            Error::WriteJustfile {
    ...
    fn list(config: &Config, mut module: &Justfile, path: &ModulePath) -> RunResult<'static> {
        ...
        for name in &path.path {
            module = module.modules.get(name).ok_or_else(|| {
        ...
        Self::list_module_vec(config, module, 0);

        Ok(())
    }

    fn list_module_vec(config: &Config, module: &Justfile, depth: usize) {
        ...
        let signature_widths = {
            ...
            if !config.list_submodules {
                for (name, _) in &module.modules {
                    signature_widths
                        .insert(name, UnicodeWidthStr::width(format!("{name} ...").as_str()));
                }
            }
            ...
            if let Some(submodules) = submodule_groups.get(&group) {
                for (i, submodule) in submodules.iter().enumerate() {
                    if config.list_submodules {
                        ...
                        Self::list_module_vec(config, submodule, depth + 1);
                    } else {
