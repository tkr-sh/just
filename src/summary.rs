...
pub fn summary(path: &Path) -> io::Result<Result<Summary, String>> {
    ...
    match Compiler::compile(&loader, path) {
        ...
        Err(error) => {
            Ok(Err(if let Error::Compile { compile_error } = error {
...
impl Fragment {
    fn new(fragment: &full::Fragment) -> Self {
        match fragment {
            full::Fragment::Text { token } => {
                Self::Text {
                    text: token.lexeme().to_owned(),
                }
            },
            full::Fragment::Interpolation { expression } => {
                Self::Expression {
...
impl Expression {
    fn new(expression: &full::Expression) -> Self {
        ...
        match expression {
            ...
            Call { thunk } => {
                match thunk {
                    full::Thunk::Nullary { name, .. } => {
                        Self::Call {
                            name: name.lexeme().to_owned(),
                            arguments: Vec::new(),
                        }
                    },
                    full::Thunk::Unary { name, arg, .. } => {
                        Self::Call {
                            name: name.lexeme().to_owned(),
