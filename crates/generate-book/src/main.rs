...
fn main() -> Result {
    for language in [Language::English, Language::Chinese] {
        ...
        for event in Parser::new_ext(&txt, Options::all()) {
            if let Event::Start(Tag::Heading(level @ (HeadingLevel::H2 | HeadingLevel::H3), ..)) =
                event
            {
                ...
                match event {
                    ...
                    Event::End(Tag::Heading(level, ..)) => {
                        ...
                        let title = events
                            ...
                            .filter_map(|event| {
                                match event {
                                    Event::Code(content) | Event::Text(content) => {
                                        Some(content.deref())
                                    },
                                    _ => None,
                                }
                        ...
                        let slug = slug(&title);
                        let link =
                            if let HeadingLevel::H1 | HeadingLevel::H2 | HeadingLevel::H3 = level {
        ...
        for chapter in &mut chapters {
            for event in &mut chapter.events {
                if let Event::Start(Tag::Link(_, dest, _)) | Event::End(Tag::Link(_, dest, _)) =
                    event
                {
